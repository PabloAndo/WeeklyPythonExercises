...
from Exercise_1 import WPE_exercise_1
...
def test_sorting_cities(monkeypatch, capsys):
    monkeypatch.setattr(
        'sys.stdin',
        StringIO('Shanghai, China\nBeijing, China\nBeijing, China\n\n'))
    WPE_exercise_1.collect_places()
    captured_out, captured_err = capsys.readouterr()

    WPE_exercise_1.display_places()
    captured_out, captured_err = capsys.readouterr()
...
def test_sorting_countries(monkeypatch, capsys):
    ...
    print('Index Israel : ' + str(israel_index))
...
def test_counting(monkeypatch, capsys):
    ...
    WPE_exercise_1.collect_places()
    captured_out, captured_err = capsys.readouterr()
    assert len(WPE_exercise_1.visits['China']) == 2
