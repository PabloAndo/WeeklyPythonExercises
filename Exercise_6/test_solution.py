...
def test_average_age_under(people, maxage, output):
    assert solution6.average_age_under(people, maxage) == output


@pytest.mark.parametrize('people, output', [
    ({}, set()),
    (all_people, {'Python', 'cooking', 'reading',
                  'horses', 'art', 'piano', 'cooking', 'biking'})
])
def test_all_hobbies(people, output):
    assert solution6.all_hobbies(people) == output


@pytest.mark.parametrize('people, output', [
    ({}, Counter()),
    (all_people, Counter({'Python': 2, 'biking': 1, 'cooking': 4,
                          'art': 1, 'horses': 1, 'piano': 1, 'reading': 3}))
])
def test_hobby_counter(people, output):
    assert solution6.hobby_counter(people) == output
...
def test_n_most_common(people, n, output):
    assert sorted(solution6.n_most_common(people, 3)) == sorted(output)
